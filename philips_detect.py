...
def get_philips_logo_confidence(image_input, debug=False, basename="debug_logo"):
    ...
    if isinstance(image_input, str):
        ...
        if image is None:
            logger.error(f"Error: Could not load image from path: {image_input}")
            return 0.0
    elif isinstance(image_input, np.ndarray):
        image = image_input
    else:
        logging.error("Error: Invalid input type. Must be a file path or numpy array.")
        return 0.0
    ...
    if debug:
        ...
        logging.info(f"Found {len(letter_contours)} letter-like contours.")
        logging.info(f"Group BBox: w={group_w}, h={group_h}, aspect={group_aspect_ratio:.2f}")
        logging.info(f"Y-Alignment Std Dev: {y_std_dev:.2f}")
        logging.info(f"Scores -> NumLetters: {num_letters_score:.2f}, Aspect: {aspect_score:.2f}, Alignment: {alignment_score:.2f}")
        logging.info(f"Final Confidence: {final_confidence:.2f}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        logging.info("Usage: python3 logo_detector.py <path_to_image.png>")
        sys.exit(1)
